...
import { InfoBadge, InfoBadgeProps } from "./InfoBadge";
...
export const InfoOverlay: React.FC<InfoOverlayProps> = ({
  ...
  topRightVariant = "default",
  bottomLeftVariant = "default",
  bottomRightVariant = "default",
  interactive = true,
  className = "",
}) => {
  return (
    <div className={`absolute inset-0 pointer-events-none ${className}`}>
      {/* Top-left badge */}
      {topLeft && (
        <InfoBadge position="top-left" variant={topLeftVariant}>
          {topLeft}
        </InfoBadge>
      )}
      ...
      {/* Top-right badge */}
      {topRight && (
        <InfoBadge position="top-right" variant={topRightVariant}>
          {topRight}
        </InfoBadge>
      )}
      ...
      {/* Bottom-left badge */}
      {bottomLeft && (
        <InfoBadge position="bottom-left" variant={bottomLeftVariant}>
          {bottomLeft}
        </InfoBadge>
      )}
      ...
      {/* Bottom-right badge */}
      {bottomRight && (
        <InfoBadge position="bottom-right" variant={bottomRightVariant}>
          {bottomRight}
        </InfoBadge>
      )}
